...
import { LayoutDashboard, Users, Calendar, UserPlus, FileText, Settings, HeartPulse, CreditCard, Droplet, BedDouble, Wallet } from 'lucide-react';
...
const navigation = [
  ...
  { name: 'Xizmat navbati', to: '/service-queue', icon: Calendar, pageCode: 'service_queue' },
  { name: 'MRT Navbati', to: '/mrt-queue', icon: Calendar, pageCode: 'mrt_queue' },
  ...
  { name: "Xonalar ro'yxati", to: '/rooms', icon: BedDouble, pageCode: 'rooms' },
  { name: "Yotoq to'lovlari", to: '/treatment-billing', icon: CreditCard, pageCode: 'treatment_billing' },
  { name: 'Kassa', to: '/cash-register', icon: CreditCard, pageCode: 'cash_register' },
  ...
  { name: 'Shifokorlar maoshlari', to: '/doctor-salaries', icon: Wallet, pageCode: 'reports' },
  ...
  return (
    <>
      {isOpen ? (
        <button
          type="button"
          aria-label="Sidebar close overlay"
      ...
      <div
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex h-full w-64 flex-col bg-gradient-to-b from-teal-900 to-teal-800 border-r border-teal-950/30 shadow-xl transition-transform duration-200",
