...
export function RoomsList() {
  ...
  const [doctorOptions, setDoctorOptions] = useState<{id: number; full_name: string}[]>([]);
  ...
  useEffect(() => {
    loadRooms();
    apiRequest<{ results: { id: number; full_name: string }[] }>('/doctors/for-room-assignment/')
      .then((res) => {
        setDoctorOptions(res.results || []);
      })
  ...
  const groupedRooms = useMemo(() => {
    ...
    return Object.entries(byArea)
      ...
      .map(([area, list]) => ({
        area,
        rooms: [...list].sort((x, y) => (x.room_number || 0) - (y.room_number || 0)),
      }));
    ...
    try {
      await apiRequest('/treatment-referrals/', {
        ...
        body: JSON.stringify({
          ...
          ...(selectedDoctorId ? { doctor: Number(selectedDoctorId) } : {}),
        }),
      });
      setAssignStatus('success');
